...
// check
    ...
    public function edit(string $id)
    {
        ...
        // RAW SQL Query
        /* ` = Position::all();` is fetching all records from the `positions` table using
        Eloquent ORM in Laravel. It retrieves all the rows from the `positions` table and stores
        them in the `` variable, which can then be used to display or manipulate the
        position data in the application. */
        $positions = Position::all();
       /* In the line ` = Employee::find();`, the `Employee::find()` method is using
       Eloquent ORM to retrieve a specific employee record from the database based on the provided
       ``. */
        $employee= Employee::find($id);
    ...
    public function update(Request $request, string $id)
    {
        ...
        $employee->email = $request->email;
        $employee->age = $request->age;
        $employee->positions_id = $request->position;
        $employee->save();
        return redirect()->route('employees.index');
    }
